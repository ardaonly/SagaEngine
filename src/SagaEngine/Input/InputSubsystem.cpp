...
{
public:
    NativeDeviceAdapter(std::unique_ptr<::SagaEngine::Platform::IInputDevice> native)
    ...
    void Update() override
    {
        ...
        while (true) {
            ...
            e.handled = false;
            e.type = ev->pressed ? Saga::Input::InputEventType::KeyDown : Saga::Input::InputEventType::KeyUp;
...
private:
    std::unique_ptr<::SagaEngine::Platform::IInputDevice> m_native;
    DeviceId m_id;
    std::string m_name;
...
{
    auto objs = ::SagaEngine::Platform::CreatePlatformObjects();
    if (objs.window) {
...
void InputSubsystem::AttachWindow(const ::SagaEngine::Platform::NativeWindowHandle& h)
{
    m_windowHandle = h;
}
...
void InputSubsystem::TranslateEventToCommand(const Saga::Input::InputEvent& e)
{
    ...
    if (e.type == Saga::Input::InputEventType::KeyDown || e.type == Saga::Input::InputEventType::KeyUp) {
        c.payload[0] = static_cast<float>(e.key);
        c.payload[1] = e.pressed ? 1.0f : 0.0f;
    }
