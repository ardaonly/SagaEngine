...
static SagaRuntime::RuntimeLaunchOptions ParseArgs(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            launchOptions.serverHost = argv[++i];
        }
        else if (arg == "--port" && i + 1 < argc)
        {
            launchOptions.serverPort =
        ...
        {
            launchOptions.packageManifestPath = std::filesystem::path(argv[++i]);
        }
        else if (arg == "--package-base-directory" && i + 1 < argc)
        {
            launchOptions.packageBaseDirectory =
...
int main(int argc, char* argv[])
{
    ...
    auto launchOptions = ParseArgs(argc, argv);
    auto startupReport = PrepareRuntimeStartup(launchOptions);
