...
///          ClientId → ServerAuthority::ServerConnection. It mediates between ZoneServer's accept
...
struct ConnectionManagerConfig
{
    ...
    uint32_t heartbeatIntervalMs{1000};     ///< Forwarded to each ServerAuthority::ServerConnection
    uint32_t connectionTimeoutMs{10000};    ///< Forwarded to each ServerAuthority::ServerConnection
    uint32_t handshakeTimeoutMs{5000};      ///< Forwarded to each ServerAuthority::ServerConnection
...
public:
    ...
    /// ServerAuthority::ServerConnection and inserts it into the session table.
