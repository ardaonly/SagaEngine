...
enum class NativeWindowBackend : uint8_t
{
    Unknown = 0,
    Win32,
    X11,
...
/// Opaque platform-window payload passed to graphics backends.
///
/// The pointer returned by IWindow::GetOSNativeHandle() remains owned by the
/// window object and is valid until Shutdown().
struct NativeWindowHandle
{
    NativeWindowBackend backend       = NativeWindowBackend::Unknown;
    void*               window        = nullptr; ///< HWND / NSWindow* or toolkit window pointer.
    void*               display       = nullptr; ///< Display* / wl_display* where applicable.
    void*               surface       = nullptr; ///< wl_surface* where applicable.
    void*               xcbConnection = nullptr; ///< xcb_connection_t* when available.
    uintptr_t           windowId      = 0;       ///< X11 Window/XID when applicable.
};
...
public:
    ...
    /// Returns a pointer to NativeWindowHandle with OS-level window data:
    /// HWND on Windows, X11 Display + Window on Linux/X11, wl_display +
    /// wl_surface on Wayland, NSWindow* on macOS. Returns nullptr if unavailable.
