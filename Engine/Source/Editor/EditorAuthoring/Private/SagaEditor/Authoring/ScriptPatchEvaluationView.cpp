...
[[nodiscard]] ScriptSourceSpanView ParseEvaluationSpan(const Json& evaluation)
{
    ScriptSourceSpanView span;
    span.startByte = IntField(evaluation, "startByte");
    span.endByte = IntField(evaluation, "endByte");
    return span;
}
                                ...
                                std::vector<AuthoringDiagnostic>& diagnostics)
{
    ...
    if (!input.is_open())
    {
        diagnostics.push_back(AuthoringDiagnostic{
            ...
            "patch_evaluation.json is missing",
            path.string(),
        });
        return Json();
    }
...
void LoadDiagnostics(const Json& json, ScriptPatchEvaluationView& view)
{
...
ScriptPatchEvaluationView LoadScriptPatchEvaluationView(
    const ProjectReadinessView& project)
{
    ScriptPatchEvaluationView view;
    if (!project.ok)
    {
    ...
    const Json json = ReadJsonFile(
        ArtifactRoot(project) / "patch_evaluation.json",
        view.diagnostics);
    if (!json.is_object())
    {
        return view;
    ...
    {
        view.span = ParseEvaluationSpan(*evaluationIt);
