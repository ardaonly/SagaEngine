...
internal static class Phase149Reports
{
    private static readonly EvidenceSpec[] PrerequisiteEvidence =
    [
        new("source-truth-inventory", "Phase 126-133", "source-truth-inventory", "source_truth_inventory_report.json", true),
        new("source-truth-gate", "Phase 126-133", "source-truth-gate", "source_truth_gate_report.json", true),
        new("asset-manifest-inventory", "Phase 134-140", "asset-source-manifest-inventory", "asset_source_manifest_inventory_report.json", true),
        new("asset-reference-gate", "Phase 134-140", "asset-reference-gate", "asset_reference_gate_report.json", true),
        new("freshness", "Phase 134-140", "generated-freshness-gate", "generated_artifact_freshness_report.json", true),
        new("scene-validation", "Phase 134-140", "scene-entity-validate", "scene_entity_validation_report.json", true),
        new("component-ownership", "Phase 141-148", "component-metadata-ownership", "component_metadata_ownership_report.json", true),
        new("editor-inspection", "Phase 141-148", "editor-inspection-model", "editor_inspection_model_report.json", true),
        new("editor-read", "Phase 141-148", "editor-source-truth-read", "editor_source_truth_read_report.json", true),
        new("runtime-audit", "Phase 141-148", "runtime-read-model-audit", "runtime_read_model_audit_report.json", true),
        new("runtime-readiness", "Phase 141-148", "runtime-readiness-gate", "runtime_readiness_gate_report.json", true),
        new("package-alignment", "Phase 146", "source-truth-alignment", "package_source_truth_alignment_report.json", true),
        new("launch-alignment", "Phase 147", "source-truth-alignment", "launch_source_truth_alignment_report.json", true),
        new("source-truth-scenario", "Phase 148", "source-truth-scenario", "source_truth_scenario_report.json", true),
    ];

    private static readonly EvidenceSpec[] ClosureEvidence =
    [
        .. PrerequisiteEvidence,
        new("client-preview-prerequisite", "Phase 149", "client-preview-prerequisite-audit", "client_preview_prerequisite_audit_report.json", true),
        new("clienthost-boundary-plan", "Phase 150", "clienthost-boundary-plan", "clienthost_boundary_plan_report.json", true),
        new("asset-import-cook-deferral", "Phase 151", "asset-import-cook-deferral", "asset_import_cook_deferral_report.json", true),
        new("broad-test-health-preflight", "Phase 152", "broad-test-health-preflight", "broad_test_health_preflight_report.json", false),
    ];
    ...
    public static ClientPreviewPrerequisiteAuditReport BuildClientPreviewPrerequisiteAudit(ManifestLoadResult load, string evidenceRoot)
    {
        ...
        if (present.Count == 0)
        {
            diagnostics.Add(Error(
                "Project.ClientPreviewPrerequisite.EvidenceMissing",
                "ClientPreviewPrerequisite",
                NormalizeRelative(evidenceRoot),
                "Client Preview prerequisite audit requires prior source-truth evidence."));
        }

        var checks = new List<SourceTruthGateCheck>
        {
            Check("SceneEntitySourceTruth", EvidenceCheck(evidence, "scene-validation"), "Scene/entity source truth evidence is present and not treated as Client Preview implementation."),
            ...
            Check("LaunchAlignment", EvidenceCheck(evidence, "launch-alignment"), "Launch alignment evidence is read-only and preserves Client Preview deferral."),
        ...
        return new ClientPreviewPrerequisiteAuditReport
        {
            Command = "client-preview-prerequisite-audit",
            ...
            [
                new FutureSeamItem
                {
                    SeamName = "ClientPreviewRuntimeReadSeam",
                    Scope = "Future planning seam from source truth evidence into a bounded Client Preview read path.",
                    Status = "PlanningOnly",
                },
            ],
            RecommendedNextSlices = ["Client Preview / Runtime Read Seam planning slice"],
    ...
    public static ClientHostBoundaryPlanReport BuildClientHostBoundaryPlan(ManifestLoadResult load, string prerequisitePath)
    {
        ...
        if (prerequisite is null)
        {
            diagnostics.Add(Error(
                ...
                "ClientHostBoundary",
                prerequisitePath,
                "ClientHost boundary planning requires the Client Preview prerequisite audit."));
        }

        var failed = prerequisite is null || prerequisiteStatus == "Failed" || prerequisiteOutcome is "Blocked" or "MissingEvidence";
        var checks = new List<SourceTruthGateCheck>
        {
            Check("PrerequisiteEvidencePresent", prerequisite is null ? "Failed" : prerequisiteStatus, "Client Preview prerequisite report is present."),
            Check("FutureSeamNamed", "Passed", "Future seam is named ClientPreviewRuntimeReadSeam."),
        ...
        return new ClientHostBoundaryPlanReport
        {
            ...
            FutureSeamName = "ClientPreviewRuntimeReadSeam",
            ClientPreviewStatus = "Deferred",
    ...
    public static Hedef4EvidenceMatrixReport BuildHedef4EvidenceMatrix(ManifestLoadResult load, string evidenceRoot)
    {
        var diagnostics = new List<ProjectDiagnostic>(load.Diagnostics);
        ...
        return new Hedef4EvidenceMatrixReport
        {
            Command = "hedef4-evidence-matrix",
    ...
    public static Hedef4ClosureReport BuildHedef4Closure(ManifestLoadResult load, string evidenceRoot)
    {
        var diagnostics = new List<ProjectDiagnostic>(load.Diagnostics);
        var evidence = BuildMatrix(load, evidenceRoot, diagnostics);
        ...
        return new Hedef4ClosureReport
        {
            Command = "hedef4-closure",
            Status = blocked ? "Failed" : outcome,
            Outcome = outcome,
            EvidenceRoot = NormalizeRelative(evidenceRoot),
            ...
            NextTargetRecommendations = ["Future Client Preview / Runtime Read Seam planning only."],
            NonClaims = NonClaims,
            MutatesSource = false,
            Enforcement = "ReportOnly",
        };
    ...
    private static List<EvidenceMatrixItem> BuildMatrix(ManifestLoadResult load, string evidenceRoot, List<ProjectDiagnostic> diagnostics)
    {
        ...
        {
            EvidenceId = item.EvidenceId,
            PhaseRange = ClosureEvidence.First(spec => spec.Id == item.EvidenceId).PhaseRange,
            Path = item.Path,
        ...
        {
            diagnostics.Add(Error(
                "Project.Hedef4Evidence.RequiredEvidenceMissingOrFailed",
                "Hedef4Evidence",
                blocked.Path,
        ...
        List<ProjectDiagnostic> diagnostics)
    {
        ...
        foreach (var spec in specs.OrderBy(item => item.Id, StringComparer.Ordinal))
        {
            ...
            var report = LoadJsonObject(path, diagnostics, $"Project.Hedef4Evidence.{spec.Id}.Malformed", missingIsDiagnostic: false);
    ...
    private static string CategoryForCode(string code)
    {
        ...
        {
            return "Hedef4Closure";
        }
        return "Hedef4Evidence";
    }
