...
|---|---|---|---|
| CBI-001 | report-only legacy violation | `cmake/modules/SagaTargets.cmake` still uses recursive source collection for several major targets; `SagaProductLib` now uses an explicit, responsibility-grouped source list. | Misplaced files can silently enter the remaining recursive targets; Apps/Saga layout and ownership guards protect the Product source list. |
...
| CBI-006 | Product Launcher Foundation ownership | `SagaProductLib` owns Qt-independent launcher state/controller/catalog/target/report services and the thin Qt Widgets renderer. Product sources are physically split by responsibility under `Apps/Saga`; `Apps/Saga/Processes/SagaProcessService.cpp` remains the sole `QProcess` owner. | Product, offscreen UI, and architecture tests guard the exact action allowlist, disabled unsupported targets, no raw process UI, source-folder ownership, and unchanged app dependency direction. |
