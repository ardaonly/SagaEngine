...
| --- | --- | --- | --- |
| `Saga` product layer | Product shell, canonical project catalog/recent state, typed launcher controller, bounded process handoff, diagnostics, and product checks | Explicit external Editor handoff, bounded StarterArena actions, async cancellation, known-report/distribution readers, and schema-2 report-only workflow status exist | Finished launcher/dashboard, generic Runtime/server/world/cloud action, package readiness, or end-to-end workflow |
