# Product History

This directory preserves older product-track and alpha/technical-preview
documents that used to live under `docs/product`.

They are historical context. Do not read them as current product promises or
current onboarding paths. The current product entry points are:

- [../../product/WHAT_IS_SAGAENGINE.md](../../product/WHAT_IS_SAGAENGINE.md)
- [../../product/CURRENT_CAPABILITIES.md](../../product/CURRENT_CAPABILITIES.md)
- [../../product/WHAT_IS_NOT_IMPLEMENTED.md](../../product/WHAT_IS_NOT_IMPLEMENTED.md)
- [../../product/CURRENT_DISTRIBUTION_STATUS.md](../../product/CURRENT_DISTRIBUTION_STATUS.md)
- [../../product/GETTING_STARTED.md](../../product/GETTING_STARTED.md)

Recently retired product-surface notes preserved here:

- [NEXT_PRODUCT_SLICE.md](NEXT_PRODUCT_SLICE.md)
- [TECHNICAL_PREVIEW_CANDIDATE.md](TECHNICAL_PREVIEW_CANDIDATE.md)
