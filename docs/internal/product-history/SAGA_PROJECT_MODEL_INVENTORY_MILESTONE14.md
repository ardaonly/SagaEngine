...
> Milestone 14 evidence for Target 1 / Technical Preview / Block B.
...
SagaProjectKit and future product milestones.

Milestone 15 ownership:
...
- `saga.project.json` and `.sagaproj` both describe project identity. Milestone 15
  does not merge them.
...
- Existing package manifests are runtime/package contracts, not project
  profile contracts.
- Launch profiles in Milestone 19 are declarative only and are not consumed by a
  launcher.
- Package profiles in Milestone 19 are declarative only and are not consumed by
  package staging or publish tooling.

## Migration Risks

- Existing editor/product shell code may continue to require
  `saga.project.json` until a future integration milestone.
- Tooling that assumes `<Project>/Build` or `<Project>/Packages` may need a
  future resolver bridge.
- Future launch/package milestones must avoid treating Milestone 19 placeholder profile
